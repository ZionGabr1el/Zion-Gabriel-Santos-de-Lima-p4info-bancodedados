...
db.serialize(() => {
    ...
    db.run('insert into TB_PESSOAS (name) values (?)', ['Jorge Jubilu'], (error) => {
        if (error) console.log(error);
    });

    db.run('insert into TB_PESSOAS (name) values (?)', ['Victor Tavares'], (error) => {
        if (error) console.log(error);
    });

    db.run('insert into TB_PESSOAS (name) values (?)', ['Giselle Rodrigues'], (error) => {
        if (error) console.log(error);
    });
    ...
    db.run('UPDATE "TB_PESSOAS" SET name= "Zion Gabriel" WHERE id=1', (error) => {
        if (error) console.log(error);
    });
